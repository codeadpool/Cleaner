...
def same_file_or_not(source: Path, destination_path: Path):
    ...
    def on_modified(self, event):
        for child in self.watch_path.iterdir():
            if child.is_file() and child.suffix.lower() in extension_paths:
                dpath = self.destination_root / extension_paths[child.suffix.lower()]
                dpath.mkdir(parents= True, exist_ok= True)
                dpath = same_file_or_not(source=child, destination_path=dpath)
                shutil.move(src=child, dst=dpath)
...
if __name__ == '__main__':
    ...
    observer = Observer()
    observer.schedule(event_handler, f'{opath}', recursive=True)
    observer.start()
    try:
        while True:
            sleep(60)
